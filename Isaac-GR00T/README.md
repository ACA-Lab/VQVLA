...
- [Quantized LIBERO Inference](#quantized-libero-inference)
...
- Python 3.10 and `uv`.
- A LIBERO checkpoint under `checkpoints/GR00T-N1.7-LIBERO/`.
- Quantization archives under `quant_weight/`.

Clone the umbrella repository with its simulator dependencies, then enter this
model directory:
...
The evaluator-only dependency is declared in `pyproject.toml`, so no separate
requirements file is needed after `uv sync`.

### Checkpoint and Weight Layout
...
| `libero_goal` | `gr00t_goal_gptvq_hdiag_4bit_b256_k256.pt` | `gr00t_goal_gptvq_hdiag_3bit_b128x128_cb64_v2.pt` |
| `libero_10` | `gr00t_10_gptvq_hdiag_4bit_b256_k256.pt` | `gr00t_10_gptvq_hdiag_3bit_b128x128_cb64_v2.pt` |

The existing `apply_gptvq(mode="load")` path loads these archives without
modifying an original checkpoint or any archive.
...
NO_ALBUMENTATIONS_UPDATE=1 MPLCONFIGDIR=/tmp/gr00t-mpl \
...
In a second terminal, run the evaluator. It writes compact JSON and routing
workbook reports after each task, without videos.
...
suites. Use `--episodes 3` for a short screening run, and stop the server with
