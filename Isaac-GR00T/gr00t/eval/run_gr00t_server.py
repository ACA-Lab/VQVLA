...
class ServerConfig:
    ...
    quantized_weight_path: str | None = None
    """Optional GPTVQ archive to load into model1 before serving."""

    seed: int | None = None
    """Optional inference RNG seed for reproducible evaluation."""

    routing_threshold: float | None = None
    """Optional override for the model1/model2 routing threshold."""

    disable_libero_router: bool = False
    """Do not load the LIBERO 4bit/3bit router; useful for pure single-model inference."""
...
def main(config: ServerConfig):
    ...
    if config.model_path is not None:
        ...
        policy = Gr00tPolicy(
            ...
            load_libero_router=not config.disable_libero_router,
        )
        if config.disable_libero_router:
            print("  LIBERO router: disabled (single-model inference)")
        if config.quantized_weight_path is not None:
            quantized_weight_path = Path(config.quantized_weight_path)
            if not quantized_weight_path.is_file():
                raise FileNotFoundError(
                    f"Quantized weight file {quantized_weight_path} does not exist"
                )
            print(f"  Quantized model1 weights: {quantized_weight_path}")
            policy.model.apply_gptvq(
                mode="load",
                save_path=str(quantized_weight_path),
                verbose=False,
            )
        if config.routing_threshold is not None:
            policy.routing_threshold = config.routing_threshold
            print(f"  Routing threshold: {policy.routing_threshold}")
