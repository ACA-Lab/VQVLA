...
def run_rollout_gymnasium_policy(
    ...
    log_inference_latency: bool = False,
) -> Any:
    ...
    while completed_episodes < n_episodes:
        ...
        if log_inference_latency:
            elapsed_time = time.time() - start_event
            print(f"Inference latency: {elapsed_time * 1000:.2f} ms")
        ...
        for env_idx in range(n_envs):
            ...
            if terminations[env_idx] or truncations[env_idx]:
                ...
                if routing_log_path is not None:
                    policy.print_and_reset(routing_log_path)
...
def run_gr00t_sim_policy(
    ...
    if record_video and video_dir is None:
        if model_path:
            ...
            video_dir = f"/home/haozhe.jiang/Isaac-GR00T/video/sim_eval_videos_{model_slug}_ac{n_action_steps}_{uuid.uuid4()}"
        else:
            video_dir = f"/home/haozhe.jiang/Isaac-GR00T/video/sim_eval_videos_{env_name}_ac{n_action_steps}_{uuid.uuid4()}"
    wrapper_configs = WrapperConfigs(
        video=VideoConfig(
            video_dir=video_dir,
    ...
    results = run_rollout_gymnasium_policy(
        ...
        log_inference_latency=log_inference_latency,
    )
    if record_video:
...
class RolloutConfig:
    ...
    routing_log_path: str | None = None
    """Optional workbook used to record model1/model2 routing counts."""

    log_inference_latency: bool = False
    """Whether to print one latency line for every policy call."""
...
if __name__ == "__main__":
    ...
    results = run_gr00t_sim_policy(
        ...
        routing_log_path=args.routing_log_path,
        log_inference_latency=args.log_inference_latency,
    )
