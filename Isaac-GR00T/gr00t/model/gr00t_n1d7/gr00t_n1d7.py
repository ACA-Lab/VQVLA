...
import tree
import threading
from gr00t.model.gr00t_n1d7.elem_quantize import apply_blockwise_int4_quant_to_named_linears, apply_blockwise_int3_quant_to_named_linears
from gr00t.model.gr00t_n1d7.vec_quant_save import (
    apply_blockwise_gptvq_to_named_linears,
    export_blockwise_gptvq_to_file,
    load_blockwise_quantized_weights_from_file,
)
...
class Gr00tN1d7(PreTrainedModel):
    ...
    def __init__(
        ...
        self._weight_quantized_once = False
        self._weight_quant_lock = threading.Lock()

    def _maybe_apply_one_time_blockwise_int4_quant(self):
        if self._weight_quantized_once:
            return
        with self._weight_quant_lock:
            if self._weight_quantized_once:
                return

            logger.info("[INT4-QDQ] Applying one-time blockwise 4-bit quant-dequant...")
            apply_blockwise_int4_quant_to_named_linears(
                self,
                block_size=256,
                verbose=True,
            )
            logger.info("[INT4-QDQ] Done.")

            self._weight_quantized_once = True

    def _maybe_apply_one_time_blockwise_int3_quant(self):
        if self._weight_quantized_once:
            return
        with self._weight_quant_lock:
            if self._weight_quantized_once:
            ...
            logger.info("[INT3-QDQ] Applying one-time blockwise 3-bit quant-dequant...")
            apply_blockwise_int3_quant_to_named_linears(
                self,
                block_size=128,
                verbose=True,
            )
            logger.info("[INT3-QDQ] Done.")

            self._weight_quantized_once = True

    def _maybe_apply_one_time_blockwise_gptvq(self):
        if self._weight_quantized_once:
            return
        ...
        with self._weight_quant_lock:
            ...
            logger.info(
                "[GPTVQ] Applying one-time blockwise vector quantization "
                "(block=256x256, codebook_size=256, vq_dim=2)..."
            )

            num_quantized = apply_blockwise_gptvq_to_named_linears(
                self,
                block_rows=256,
                block_cols=256,
                codebook_size=256,
            ...
            logger.info(f"[GPTVQ] Done. Quantized {num_quantized} linear layers.")
            self._weight_quantized_once = True

    def apply_weight_quantization(self):
        self._maybe_apply_one_time_blockwise_gptvq()
    ...
    def apply_gptvq(
        ...
        参数:
        mode:
            - "load":   从 save_path 加载量化后的权重并覆盖当前模型
            - "export": 对当前模型做量化，保存到 save_path，并加载量化权重
            - "apply":  对当前模型直接现算现替换，不保存文件
        save_path:
            - mode="load" 或 "export" 时需要提供
            - mode="apply" 时可忽略
        """
        import os
        import logging
        logger = logging.getLogger(__name__)
        if mode not in {"load", "export", "apply"}:
            raise ValueError(
                f"Invalid mode={mode}, expected one of: 'load', 'export', 'apply'"
            )
        if mode in {"load", "export"} and not save_path:
            raise ValueError(f"mode={mode} requires a non-empty save_path")
        if verbose:
            logger.warning(
                "[GPTVQ][entry] mode=%s file=%s block=(%dx%d) codebook_size=%d "
                "vq_dim=%d kmeans_iters=%d skip_incomplete_blocks=%s",
                mode,
                save_path,
                block_rows,
                block_cols,
                ...
                vq_dim,
                kmeans_iters,
                skip_incomplete_blocks,
            )
        if mode == "load":
            if not os.path.exists(save_path):
                raise FileNotFoundError(
                    f"[GPTVQ][entry] quantized file not found: {save_path}"
                )
            load_blockwise_quantized_weights_from_file(
                root_module=self,
                save_path=save_path,
    ...
    def get_action(self, inputs: dict, options: dict[str, Any] | None = None) -> BatchFeature:
        ...
        # starterb = torch.cuda.Event(enable_timing=True)
        # enderb = torch.cuda.Event(enable_timing=True)
        # starterb.record()
        ...
        # print(f"backbone: {starterb.elapsed_time(enderb):.3f} ms")
        # startera = torch.cuda.Event(enable_timing=True)
        # endera = torch.cuda.Event(enable_timing=True)
        # startera.record()
        ...
        # endera.record()
        # torch.cuda.synchronize()
        # print(f"action: {startera.elapsed_time(endera):.3f} ms")

        # print("datatype: ", self.backbone.model.model.visual.blocks[0].attn.qkv.weight.data.dtype)
        # print("action_shape: ", action_outputs["action_pred"].shape)
