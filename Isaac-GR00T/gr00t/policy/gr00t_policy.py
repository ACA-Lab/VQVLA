...
class Gr00tPolicy(BasePolicy):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            load_libero_router: Whether to load the LIBERO 4bit/3bit router.
        """
        ...
        # Restore the two-model router for the four LIBERO checkpoints.  The
        # model1 archive is the better 4bit result from the fixed-seed rollout
        # comparison; model2 receives the corresponding GPTVQ 3bit Hessian-
        # diagonal archive (block 128x128, codebook 64, vector length 2).
        # Loading is QDQ BF16, so both models retain the same runtime dtype and
        # architecture.
        quant_weight_dir = Path("quant_weight")
        model1_weight_names = {
            "libero_spatial": "gr00t_spatial_gptvq_4bit_b256_k256.pt",
            "libero_object": "gr00t_object_gptvq_hdiag_4bit_b256_k256.pt",
            "libero_goal": "gr00t_goal_gptvq_hdiag_4bit_b256_k256.pt",
            "libero_10": "gr00t_10_gptvq_hdiag_4bit_b256_k256.pt",
        }
        model2_weight_names = {
            "libero_spatial": "gr00t_spatial_gptvq_hdiag_3bit_b128x128_cb64_v2.pt",
            "libero_object": "gr00t_object_gptvq_hdiag_3bit_b128x128_cb64_v2.pt",
            "libero_goal": "gr00t_goal_gptvq_hdiag_3bit_b128x128_cb64_v2.pt",
            "libero_10": "gr00t_10_gptvq_hdiag_3bit_b128x128_cb64_v2.pt",
        }
        if load_libero_router and model_dir.name in model1_weight_names:
            model1_weight_path = quant_weight_dir / model1_weight_names[model_dir.name]
            model2_weight_path = quant_weight_dir / model2_weight_names[model_dir.name]
            self.model.apply_gptvq(
                mode="load",
                save_path=str(model1_weight_path),
                verbose=False,
            )

            model2 = AutoModel.from_pretrained(model_dir)
            model2.eval()
            model2.to(device=device, dtype=torch.bfloat16)
            model2.apply_gptvq(
                mode="load",
                save_path=str(model2_weight_path),
                verbose=False,
            )
            self.model2 = model2
        else:
            # Keep non-LIBERO checkpoints usable when this local router is
            # present but no matching quantized archives exist.
            self.model2 = self.model

        self.last_action = -1.0
        self.routing_threshold = {
            "libero_spatial": float("-inf"),
            "libero_object": float("-inf"),
            "libero_goal": float("-inf"),
            "libero_10": float("-inf"),
        }.get(model_dir.name, 100.7)
        self.exe_stage = 0
        self.tran_stage = 0
    ...
    def print_and_reset(self, xlsx_file_name: str) -> None:
        self.last_action = -1.0
        workbook = openpyxl.load_workbook(xlsx_file_name)
        sheet = workbook["Sheet1"]
        total_calls = self.exe_stage + self.tran_stage
        new_row = [
            self.exe_stage,
            self.tran_stage,
            total_calls,
            self.exe_stage / total_calls if total_calls else 0.0,
        ]
    ...
    def _get_action(
        ...
        with torch.inference_mode():
            if self.last_action < self.routing_threshold:
                model_pred = self.model.get_action(**collated_inputs)
                self.exe_stage += 1
            else:
                model_pred = self.model2.get_action(**collated_inputs)
                self.tran_stage += 1
        ...
        x = casted_action['x'].mean().item()
        y = casted_action['y'].mean().item()
        z = casted_action['z'].mean().item()
        self.last_action = math.sqrt(x * x + y * y + z * z)
...
class Gr00tSimPolicyWrapper(PolicyWrapper):
    ...
    **Important**: If you are using other environments, custom robots, or building new environments,
    ...
    def print_and_reset(self, xlsx_file_name: str) -> None:
        return self.policy.print_and_reset(xlsx_file_name=xlsx_file_name)
